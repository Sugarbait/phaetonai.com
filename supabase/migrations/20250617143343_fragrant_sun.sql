...
-- Drop existing tables if they exist (to handle any partial creation issues)
DROP TABLE IF EXISTS analytics_hourly_stats CASCADE;
DROP TABLE IF EXISTS analytics_geolocations CASCADE;
...
CREATE TABLE analytics_geolocations (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  session_id text NOT NULL,
  country text,
  country_code text,
...
CREATE INDEX idx_analytics_visitors_date ON analytics_visitors(date_visited);
CREATE INDEX idx_analytics_visitors_session ON analytics_visitors(session_id);
CREATE INDEX idx_analytics_page_views_date ON analytics_page_views(date_viewed);
CREATE INDEX idx_analytics_page_views_path ON analytics_page_views(page_path);
CREATE INDEX idx_analytics_geolocations_session ON analytics_geolocations(session_id);
CREATE INDEX idx_analytics_geolocations_country ON analytics_geolocations(country);
CREATE INDEX idx_analytics_hourly_date_hour ON analytics_hourly_stats(date_hour, hour_of_day);
